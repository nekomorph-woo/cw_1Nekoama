...
## 协作模式规则
1.尽量使用简体中文回复，包括聊天内容、方案、文档、代码注释等等，并且不要翻译术语
2.除非有显式要求，尽量不要编写兼容代码
3.代码注释符合业务逻辑，不要将时间、计划等无效注释编写到代码中
4.当明确表达或是当前讨论的内容是实现一个大功能或大目标时，请通过【大目标流程】与我共同推进功能开发（使用以下流程必须向我进行确认，否则还是按照常规todo列表方式进行任务）：
- 规划阶段：使用plan mode与我讨论功能具体需求，当明确认为需求讨论清楚后，在外部文档（项目根目录/TASK_PLAN.md）输出并管理整体规划和里程碑
  🎯 大目标
  ├── 📋 子功能A
  │   ├── ✅ 任务A1 (completed)
  │   ├── 🔄 任务A2 (in_progress)  
  ...
  ├── 📋 子功能B
  └── 📋 子功能C
- 执行阶段：只将当前子功能的任务加入todo列表，若单项任务较为复杂，可以按照以下阶段安排更细粒度的任务
