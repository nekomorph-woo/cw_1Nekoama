...
class TokenStatsTab : BaseNekoamaTab() {
     ...
     * 创建Token使用统计组件
     */
    private fun createTokenUsageStats(snapshot: EnhancedMetricsSnapshot): JComponent {
        val panel = JPanel()
        panel.layout = BoxLayout(panel, BoxLayout.Y_AXIS)
     ...
     * 创建基础性能统计组件
     */
    private fun createBasicPerformanceStats(snapshot: EnhancedMetricsSnapshot): JComponent {
        val panel = JPanel()
        panel.layout = BoxLayout(panel, BoxLayout.Y_AXIS)
     ...
     * 创建详细性能统计组件
     */
    private fun createDetailedPerformanceStats(snapshot: EnhancedMetricsSnapshot): JComponent {
        val panel = JPanel()
        panel.layout = BoxLayout(panel, BoxLayout.Y_AXIS)
        ...
        scope.launch {
            try {
                ...
                val exportData = buildString {
                    ...
                    appendLine("Export Time: ${LocalDateTime.now()}")
                    appendLine()
                ...
                val stringSelection = StringSelection(exportData)
