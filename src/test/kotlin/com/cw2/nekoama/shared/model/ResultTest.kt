...
class ResultTest {
    ...
    inner class HigherOrderFunctionTests {
        ...
        fun `flatMap 方法 - 成功时应该链式调用`() {
            ...
            val flatMapped = result.flatMap { value ->
                ...
                else Result.error(NekoamaError.Unknown("negative value"))
            }
    ...
    inner class FilterAndCheckTests {
        ...
        fun `getOrNull - 错误时返回 null`() {
            val result = Result.error<String>(NekoamaError.Unknown("error"))
    ...
    inner class CompanionObjectTests {
        ...
        fun `fromNullable - 非空值返回成功`() {
            val result = Result.fromNullable("value", NekoamaError.Unknown("null value"))
        ...
        fun `fromNullable - 空值返回错误`() {
            ...
            val result = Result.fromNullable<String?>(null, error)
        ...
        fun `combine - 全部成功时返回成功`() {
            ...
            val combined = Result.combine(results)
        ...
        fun `combine 两个结果 - 全部成功时返回成功`() {
            ...
            val combined = Result.combine(result1, result2) { a, b -> a + b }
        ...
        fun `combine 两个结果 - 第一个失败时返回错误`() {
            val error = NekoamaError.Unknown("error")
            val result1 = Result.error<Int>(error)
            val result2 = Result.success(20)
            val combined = Result.combine(result1, result2) { a, b -> a + b }
        ...
        fun `combine 三个结果 - 全部成功时返回成功`() {
            val result1 = Result.success(10)
            val result2 = Result.success(20)
            val result3 = Result.success(30)
            val combined = Result.combine(result1, result2, result3) { a, b, c -> a + b + c }
        ...
        fun `combine 三个结果 - 中间失败时返回错误`() {
            val error = NekoamaError.Unknown("error")
            val result1 = Result.success(10)
            val result2 = Result.error<Int>(error)
            val result3 = Result.success(30)
            val combined = Result.combine(result1, result2, result3) { a, b, c -> a + b + c }
    ...
    inner class ExtensionFunctionTests {
        ...
        fun `toResult 扩展 - 空值返回错误`() {
            ...
            val result: Result<String?> = null.toResult(error)
